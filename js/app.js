...
if (iconMenu != null) {
	let delay = 500;
	let menuBody = document.querySelector(".top-header__body");
...
function popup_open(item, video = '') {
	...
	if (curent_popup && unlock) {
		...
		if (!document.querySelector('.top-header__body._active')) {
			body_lock_add(500);
		}
...
function popup_close(item, bodyUnlock = true) {
	if (unlock) {
		...
		if (!document.querySelector('.top-header__body._active') && bodyUnlock) {
